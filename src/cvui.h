...
/**
 ...
 \param theCreateNamedWindow if an OpenCV window named `theWindowName` should be created during the initialization. Windows are created using `cv::namedWindow()`. If this parameter is `false`, ensure you call `cv::namedWindow(windowName)` *before* initializing cvui, otherwise it will not be able to track UI interactions. 
...
/**
 ...
 \sa rect()
*/
    cv::Rect window(cv::Mat& theWhere, int theX, int theY, int theWidth, int theHeight, const cv::String& theTitle, double theFontScale = DEFAULT_FONT_SCALE);
...
// You should probably not be using anything from here.
    namespace internal
    {
        ...
        cv::Rect window(cvui_block_t& theBlock, int theX, int theY, int theWidth, int theHeight, const cv::String& theTitle, double theFontScale);
...
cv::Rect window(cv::Mat& theWhere, int theX, int theY, int theWidth, int theHeight, const cv::String& theTitle, double theFontScale) {
	internal::gScreen.where = theWhere;
    return internal::window(internal::gScreen, theX, theY, theWidth, theHeight, theTitle, theFontScale);
}

cv::Rect pad(cv::Rect& rect, double padding) {
    rect.x += padding;
    rect.y += padding;
